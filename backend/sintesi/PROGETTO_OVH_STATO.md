...
- **Volume DB (snapshot):** `specialists` in produzione **3024** righe (query 2026-04-01 su OVH).
...
*Ultimo aggiornamento significativo: **2026-04-01** — stato produzione **verificato OK** (form generico + consulenza online, mail admin To+Cc). **Frontend:** banner promo più grande/CTA; menu Servizi → **Veterinario online**; `/servizi/` con card consulenza online; deploy **`scp`** `dist/` + permessi. Tag rollback **`checkpoint/ovh-2026-04-01-email-cc-verified`**. Commit feature email **`5a58537`**; sintesi + checkpoint nel commit puntato dal tag. Form `/richiedi-assistenza/` e inline servizio×animale con password obbligatoria, `POST /requests` + redirect chat; query `servizio` risolta da slug pagina.*
...
| **Pipeline veterinari (CSV → UI + DB)** | Fonte unica consigliata: `veterinari_frontend/src/data/veterinari.csv` → build genera schede/elenco; tabella **`specialists`** popolata con [`import_from_frontend_dataset.py`](../scripts/import_from_frontend_dataset.py) (stesso CSV). Produzione: **3024** specialist; CSV ~3147 righe (header + dati, parte righe non importate per filtri). Prima di ondate grandi: **backup DB**, `--dry-run`, batch `--limit`/`--offset`. Documentazione: [`DATABASE.md`](DATABASE.md) sezione *Pipeline anagrafica veterinari*. |
