...
| **CAP Nominatim (ondata)** | [`backfill_cap_nominatim_wave.py`](../scripts/backfill_cap_nominatim_wave.py): secondo passaggio su CAP ancora vuoti via **OSM Nominatim** (online, ~1 req/s). Vedi [`DATABASE.md`](DATABASE.md) *Dati mancanti*. Telefono/indirizzo/orari: non recuperabili in modo semplice senza API a pagamento o scraping. |
