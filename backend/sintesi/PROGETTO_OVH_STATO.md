...
- **Deploy attuale:** backend aggiornabile con **`git pull`** solo se il server ha clone Git in `/var/www/veterinari`; altrimenti pacchetto (`tar` estratto in `backend/`) + **`scp`** del `dist/` frontend. Dopo ogni upload frontend: **permessi** (§2) obbligatori. UX **2026-04-01:** barra promozionale in cima più **alta** e **colorata** (sempre trasparente + blur); menu **Servizi** con sottomenu **Veterinario online**; pagina `/servizi/` con **card** in evidenza verso consulenza online.
- **Checkpoint rollback (stato buono):** tag Git **`checkpoint/ovh-2026-04-01-email-cc-verified`** → commit **`7e4add3`** (SHA: `git rev-parse checkpoint/ovh-2026-04-01-email-cc-verified^0` dopo `git fetch`). Produzione verificata: notifiche admin **To+Cc**, form generico e consulenza online. **Nota:** il tag salva solo il **codice**; un import massivo veterinari va annullato con **backup DB** (`pg_dump`), non con `git checkout` da solo.
- **Dati veterinari:** directory e mappe usano **`veterinari.csv`** (build frontend); il **matching** richieste usa **`specialists` in PostgreSQL**. Pipeline unificata: aggiornare CSV → build → `import_from_frontend_dataset.py` — dettaglio in [`DATABASE.md`](DATABASE.md) sezione *Pipeline anagrafica veterinari*.
- **Volume DB (snapshot):** `specialists` in produzione **10681** righe (query 2026-04-01 su OVH, dopo ondata Italia PG: **+7657** con `import_batch=italia_pg_2026_04`).
...
| Area | Modifica |
|------|----------|
| **CAP ondata PG (2026-04-01)** | [`backfill_cap_italia_pg_wave.py`](../scripts/backfill_cap_italia_pg_wave.py): aggiorna **solo** `specialists.cap` per `import_batch=italia_pg_2026_04` con CAP ancora vuoto, usando `comuni_italiani_full.csv` (match comune+provincia + fuzzy). **Non** tocca gli specialist pre-esistenti. In produzione: `git pull` + `python scripts/backfill_cap_italia_pg_wave.py` (prima `--dry-run`); ~**90%** delle righe onda valorizzabili; il resto resta senza CAP (frazioni / nomi località non in anagrafica comuni). |
...
| **Frontend UX (2026-04-01)** | `PromoBanner`: barra più **alta** (`min-h` ~5.5rem / 7rem), gradiente **più acceso** (sempre trasparente + `backdrop-blur-xl`), CTA più visibile; **Header** `sticky` sotto banner (`top-[5.5rem]` / `md:top-[7rem]`). Menu **Servizi** → dropdown Radix: **Tutti i servizi**, **Veterinario online** (in homepage anche **Servizi in homepage** con scroll a `#servizi`); mobile: blocco Servizi. **`/servizi/`**: card **Veterinario online** in evidenza sopra le categorie. Deploy **solo statiche:** `npm run build`, `scp` → `/var/www/veterinari/frontend/dist/`, `fix_frontend_dist_permissions.sh`; homepage **HTTP 200** |
| **Pipeline veterinari (CSV → UI + DB)** | Fonte unica consigliata: `veterinari_frontend/src/data/veterinari.csv` → build genera schede/elenco; tabella **`specialists`** popolata con [`import_from_frontend_dataset.py`](../scripts/import_from_frontend_dataset.py) (stesso CSV). Produzione: **10681** specialist (2026-04-01, inclusa ondata PG); CSV ~3147 righe (header + dati, parte righe non importate per filtri). Prima di ondate grandi: **backup DB**, `--dry-run`, batch `--limit`/`--offset`. Documentazione: [`DATABASE.md`](DATABASE.md) sezione *Pipeline anagrafica veterinari*. |
| **Ondata Italia PG (2026)** | Migrazione **`0010_specialist_import_batch`**: colonna `import_batch` su `specialists`. Script [`import_italia_pg_wave.py`](../scripts/import_italia_pg_wave.py) + `veterinari_italia_completo_dedup.csv` (root repo) → `veterinari_italia_wave.csv` + insert DB (`import_batch=italia_pg_2026_04`). Frontend: `contactLoginRequired` per `source` `italia_pg_*` (card + `ContactGate`); profilo con `getAccessToken()`. Matching/email admin invariati (`is_active` + specialty). |
| **Checkpoint rollback (2026-04-01)** | Tag annotato **`checkpoint/ovh-2026-04-01-email-cc-verified`** → commit **`7e4add3`** (SHA: `git rev-parse checkpoint/ovh-2026-04-01-email-cc-verified^{commit}`). Rollback codice/VPS: `git fetch origin tag checkpoint/ovh-2026-04-01-email-cc-verified && git checkout checkpoint/ovh-2026-04-01-email-cc-verified`, poi `pip` / `alembic` / `restart` come da [`DEPLOY_SAFE_WORKFLOW.md`](DEPLOY_SAFE_WORKFLOW.md). Tag precedenti (es. marzo 2026) restano per release più vecchie |
...
**Checkpoint Git (rollback rapido) — riferimento attuale:** tag annotato **`checkpoint/ovh-2026-04-01-email-cc-verified`** → commit **`7e4add3`** (stato **buono** verificato in produzione: email admin To+Cc, form generico e consulenza online). `git fetch origin tag checkpoint/ovh-2026-04-01-email-cc-verified` poi `git checkout checkpoint/ovh-2026-04-01-email-cc-verified`. SHA commit (tag annotato): `git rev-parse checkpoint/ovh-2026-04-01-email-cc-verified^{commit}`. Sul VPS: stesso checkout sul clone monorepo, poi `pip install`, `alembic upgrade head`, `sudo systemctl restart veterinari`, health. Per sovrascrivere `master` in emergenza solo con accordo di team: `git push --force` dopo reset locale; preferibile `git revert` dei commit indesiderati. Tag storici: **`checkpoint/ovh-2026-03-30-pre-dashboard`** (commit `bd21f78`, prima release dashboard) e riga tabella §14 *Checkpoint rollback* marzo 2026.
