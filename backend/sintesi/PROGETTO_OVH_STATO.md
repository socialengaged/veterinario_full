...
- **Checkpoint rollback (stato buono):** tag Git **`checkpoint/ovh-2026-04-01-email-cc-verified`** (SHA: `git rev-parse checkpoint/ovh-2026-04-01-email-cc-verified` dopo `git fetch`). Produzione verificata: notifiche admin **To+Cc**, form generico e consulenza online.
...
| **Checkpoint rollback (2026-04-01)** | Tag annotato **`checkpoint/ovh-2026-04-01-email-cc-verified`** (commit: `git rev-parse checkpoint/ovh-2026-04-01-email-cc-verified`). Rollback codice/VPS: `git fetch origin tag checkpoint/ovh-2026-04-01-email-cc-verified && git checkout checkpoint/ovh-2026-04-01-email-cc-verified`, poi `pip` / `alembic` / `restart` come da [`DEPLOY_SAFE_WORKFLOW.md`](DEPLOY_SAFE_WORKFLOW.md). Tag precedenti (es. marzo 2026) restano per release più vecchie |
...
**Checkpoint Git (rollback rapido) — riferimento attuale:** tag annotato **`checkpoint/ovh-2026-04-01-email-cc-verified`** (stato **buono** verificato in produzione: email admin To+Cc, form generico e consulenza online). `git fetch origin tag checkpoint/ovh-2026-04-01-email-cc-verified` poi `git checkout checkpoint/ovh-2026-04-01-email-cc-verified` (SHA: `git rev-parse checkpoint/ovh-2026-04-01-email-cc-verified`). Sul VPS: stesso checkout sul clone monorepo, poi `pip install`, `alembic upgrade head`, `sudo systemctl restart veterinari`, health. Per sovrascrivere `master` in emergenza solo con accordo di team: `git push --force` dopo reset locale; preferibile `git revert` dei commit indesiderati. Tag storici: **`checkpoint/ovh-2026-03-30-pre-dashboard`** (commit `bd21f78`, prima release dashboard) e riga tabella §14 *Checkpoint rollback* marzo 2026.
