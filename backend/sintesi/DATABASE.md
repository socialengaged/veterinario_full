...
- [`scripts/backfill_cap_nominatim_wave.py`](../scripts/backfill_cap_nominatim_wave.py) — **secondo passaggio** CAP (sempre solo `import_batch` + CAP vuoto): geocoding **OpenStreetMap Nominatim** (`città, provincia, Italia` → `postcode`). Dedup per coppia (city, province); delay ~**1,1 s** tra richieste (policy OSM). Opzioni: `--dry-run` (query senza commit), `--no-fetch` (solo conteggi), `--limit-unique-pairs N` (test). Dopo i comuni ISTAT, in produzione ha recuperato **~513** CAP aggiuntivi; **~252** righe possono restare senza CAP (stringhe non risolvibili come luogo, assenza `postcode` in OSM).
- [`scripts/scrape_paginegialle_specialists.py`](../scripts/scrape_paginegialle_specialists.py) — legge la **scheda PG** (`website_url` con `paginegialle.it`), estrae **JSON-LD** `LocalBusiness` (`app/services/pg_jsonld_parse.py`): `phone_fixed`, `phone_mobile`, `contact_email`, indirizzo/CAP, sito esterno (`sameAs`), orari, descrizione. Aggiorna `specialists` (migrazione **`0011`**). Parser testato senza rete + smoke HTTP. Background: [`deploy/run_pg_scrape_background.sh`](../deploy/run_pg_scrape_background.sh) / `.ps1`.

**Stato operativo scrape (aprile 2026):** job su **OVH** (log **`/tmp/pg_scrape_full.log`**). Dettagli: [`PROGETTO_OVH_STATO.md`](PROGETTO_OVH_STATO.md).

- [`scripts/sync_italia_wave_csv_from_db.py`](../scripts/sync_italia_wave_csv_from_db.py) — dopo lo scrape, aggiorna **`veterinari_italia_wave.csv`** dal DB (match su `website_url`): telefoni, indirizzo, sito, orari. Poi **`npm run build`** e **`scp`** `dist/` su produzione così il sito mostra i contatti aggiornati (il DB è già allineato allo scrape).
