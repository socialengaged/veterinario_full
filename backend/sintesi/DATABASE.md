...
**Stato operativo scrape (aprile 2026):** job lungo su **OVH** in corso o completabile con `nohup`; log **`/tmp/pg_scrape_full.log`**. Monitor: `grep -c ' OK ' /tmp/pg_scrape_full.log`. Ripresa: rieseguire lo **stesso** comando se il processo termina (idempotente); evitare due job paralleli. Dettagli e coerenza con email admin/mailto: [`PROGETTO_OVH_STATO.md`](PROGETTO_OVH_STATO.md) sezione *Attività in corso — scrape PagineGialle*.
