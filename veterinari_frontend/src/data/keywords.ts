...
export const cityKeywordPatterns: CityKeywordPattern[] = [
  {
    ...
    metaDescTemplate: (c) => `Cerchi un veterinario a ${c}? Consulta le strutture veterinarie disponibili nella zona. Visite, vaccinazioni, urgenze e visite a domicilio.`,
  ...
  },
  {
    ...
    metaDescTemplate: (c) => `Cerchi una clinica veterinaria a ${c}? Scopri le strutture disponibili per la cura del tuo animale: visite, chirurgia, diagnostica e pronto soccorso.`,
  ...
  {
    prefix: "veterinario-h24-",
    label: "Veterinario H24",
    serviceContext: "pronto-soccorso-veterinario",
    angle: "h24",
    h1Template: (c) => `Veterinario H24 a ${c}`,
    metaTitleTemplate: (c) => `Veterinario H24 a ${c} — Servizio veterinario continuativo`,
    metaDescTemplate: (c) => `Cerchi un veterinario aperto H24 a ${c}? Consulta le strutture con servizio continuativo e reperibilità notturna per emergenze.`,
    introTemplate: (c) => `In situazioni di emergenza, sapere dove trovare un veterinario H24 a ${c} può fare la differenza. Le strutture con servizio continuativo o reperibilità notturna gestiscono urgenze che non possono attendere gli orari ordinari di apertura.`,
    summaryTemplate: (c) => `Veterinario aperto H24 a ${c}: consulta strutture con servizio continuativo, pronto soccorso e reperibilità notturna.`,
  },
  {
    prefix: "pronto-soccorso-veterinario-",
    label: "Pronto soccorso veterinario",
    serviceContext: "pronto-soccorso-veterinario",
    angle: "emergency",
    h1Template: (c) => `Pronto soccorso veterinario a ${c}`,
    metaTitleTemplate: (c) => `Pronto Soccorso Veterinario a ${c} — Emergenze animali`,
    metaDescTemplate: (c) => `Pronto soccorso veterinario a ${c}: strutture disponibili per emergenze, traumi, avvelenamenti e urgenze notturne.`,
    introTemplate: (c) => `Se il tuo animale ha bisogno di cure urgenti a ${c}, è fondamentale sapere a chi rivolgersi. Il pronto soccorso veterinario gestisce emergenze come traumi, avvelenamenti, difficoltà respiratorie e altre situazioni critiche che richiedono intervento immediato.`,
    summaryTemplate: (c) => `Pronto soccorso veterinario a ${c}: strutture disponibili per traumi, avvelenamenti e emergenze.`,
  },
];
...
export const animalCityKeywordPatterns: AnimalCityKeywordPattern[] = [
  {
    ...
    metaDescTemplate: (c) => `Cerchi un veterinario per cani a ${c}? Consulta le strutture disponibili per visite, vaccinazioni, urgenze e cure per il tuo cane.`,
...
export const animalKeywordPages: Record<string, AnimalKeywordPage> = {
  "veterinario-cane": {
    ...
    metaDescription: "Cerchi un veterinario per cani? Consulta le strutture disponibili: visite, vaccinazioni, chirurgia, urgenze e servizi per il tuo cane.",
    ...
    whenToSeek: "Porta il tuo cane dal veterinario per le visite annuali, il piano vaccinale, quando noti cambiamenti nel comportamento o nell'appetito, per problemi dermatologici o ortopedici, e in caso di emergenza.",
    commonServices: ["visita-veterinaria", "vaccinazioni", "chirurgia-veterinaria", "pronto-soccorso-veterinario", "veterinario-a-domicilio"],
    faq: [
      ...
      { q: "Come scelgo il veterinario per il mio cane?", a: "Considera la vicinanza, gli orari di apertura, la disponibilità per urgenze, e la possibilità di visite a domicilio. Puoi consultare l'elenco delle strutture disponibili nella tua zona." },
    ],
  },
  "veterinario-gatto": {
  ...
  "veterinario-cavallo": {
    ...
    metaDescription: "Cerchi un veterinario per cavalli? Trova un veterinario equino per visite, vaccinazioni, cure dentali e emergenze per i tuoi cavalli.",
    h1: "Veterinario per cavalli",
    summary: "Consulta le strutture veterinarie equine nella tua zona: visite, cure dentali, vaccinazioni e servizi per emergenze ippiche.",
  ...
  "veterinario-uccelli": {
    ...
    whenToSeek: "Porta il tuo uccello dal veterinario aviare per i controlli periodici, quando noti variazioni nell'appetito, nel piumaggio, nella respirazione o nel comportamento, e per le emergenze.",
  ...
  "veterinario-animali-esotici": {
    ...
    whenToSeek: "Cerca un veterinario per animali esotici quando acquisti o adotti un nuovo animale, per i controlli periodici, quando noti comportamenti anomali, problemi alimentari, alterazioni fisiche, o in caso di emergenza.",
...
export const specialtyKeywordPages: Record<string, SpecialtyKeywordPage> = {
  ...
  "veterinario-oculista": {
    ...
    intro: "L'oftalmologia veterinaria si occupa delle malattie degli occhi negli animali domestici. I problemi oculari possono variare da condizioni lievi a emergenze che richiedono un intervento immediato. Un oftalmologo veterinario dispone di strumentazione specifica per una diagnosi accurata e trattamenti mirati.",
  ...
  "veterinario-notturno": {
    ...
    specialtyName: "Veterinario notturno",
    emoji: "🌙",
    metaTitle: "Veterinario Notturno — Assistenza veterinaria di notte",
    metaDescription: "Cerchi un veterinario notturno? Trova strutture con servizio notturno e reperibilità per emergenze veterinarie nelle ore serali e notturne.",
    h1: "Veterinario notturno",
    summary: "Trova un veterinario con servizio notturno o reperibilità: assistenza per emergenze veterinarie nelle ore serali e notturne.",
    intro: "Le emergenze veterinarie non rispettano gli orari d'ufficio. Sapere dove trovare un veterinario disponibile di notte può fare la differenza in situazioni critiche. Non tutte le strutture offrono un servizio notturno continuativo: alcune garantiscono la reperibilità telefonica, altre hanno turni notturni con personale in sede.",
    whatTheyDo: "I servizi veterinari notturni gestiscono emergenze che si presentano fuori dall'orario ordinario: traumi, avvelenamenti, difficoltà respiratorie, crisi convulsive e altre urgenze che non possono attendere il mattino.",
    whenToSeek: "Cerca un veterinario notturno quando il tuo animale presenta un'emergenza medica nelle ore serali o notturne e non puoi attendere l'apertura del tuo veterinario abituale.",
    whatToExpect: "Il servizio notturno è dedicato alle emergenze. Il veterinario effettuerà una valutazione rapida, stabilizzazione e trattamento dell'urgenza. Per follow-up e cure non urgenti, ti indirizzerà al veterinario di riferimento.",
    relatedServices: ["pronto-soccorso-veterinario", "veterinario-a-domicilio"],
    faq: [
      { q: "Come trovo un veterinario aperto di notte?", a: "Non tutte le strutture offrono servizio notturno. Puoi consultare le strutture disponibili nella tua zona, incluse le cliniche con reperibilità." },
      { q: "Il servizio notturno costa di più?", a: "Sì, generalmente le tariffe per il servizio notturno e festivo sono superiori a quelle ordinarie per coprire i costi del personale fuori orario." },
    ],
  },
};
...
export const serviceCityKeywordPatterns: ServiceCityKeywordPattern[] = [
  ...
  },
  {
    ...
    metaDescTemplate: (c) => `Cerchi un chirurgo veterinario a ${c}? Consulta le strutture per sterilizzazioni, interventi ortopedici, chirurgia dei tessuti molli e urgenze.`,
    ...
    summaryTemplate: (c) => `Chirurgo veterinario a ${c}: sterilizzazioni, ortopedia, tessuti molli e urgenze chirurgiche.`,
  },
];
